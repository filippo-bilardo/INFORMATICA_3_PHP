...
function sec_session_start() {
  ...
  $httponly = true; // Questo impedirà ad un javascript di accedere all'id di sessione.
...
function reset_password($userid, $password, $cpassword) {
  ...
  $crypt_pwd = hash('sha512', $password.$salt);
  ...
  try {
    ...
    $retval = $stmt->bindParam(':password', $crypt_pwd, PDO::PARAM_STR);
...
function login($email, $password) {
  ...
  try {
    ...
    if($rowCount == 1) {
      ...
      $db_salt = $row['salt'];
      $password_enc = hash('sha512', $password.$db_salt); // codifica la password usando una chiave univoca.
...
{
    public function __construct($file = 'app_settings.ini')
    {
...
#4. Crea la tabella di log che memorizzerà tutti i tentativi di login.
